...
- **APK:** `D:\MyProjects\Sarathi\app\build\outputs\apk\debug\app-debug.apk`
...
3. **Kotlin compile flake (Windows):** First `clean :app:assembleDebug` failed with Kotlin incremental cache / “Could not delete … kotlin-classes” errors. Addressed with clean `app\build` and **`kotlin.incremental=false`** in `gradle.properties` (see below). Consider re-enabling incremental later once caches are stable.
