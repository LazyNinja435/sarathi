...
- SQLite in `knowledge/indexes` and `app/src/main/assets/rag` are byte-identical (**44,838,912** bytes) after index.  
...
- `app/src/main/java/com/sarathi/app/rag/SarathiDatabaseProvider.kt` — If an internal DB file exists but `SQLiteDatabase.openDatabase` fails, delete the file once and recopy from assets.
...
- `knowledge/sources/gita/processed/gita_verses.jsonl`, `gita_validation_report.json`, `gita_chapters.json`, related outputs  
- `knowledge/indexes/sarathi_rag.sqlite`, `sarathi_rag_manifest.json`, `build_report.json`, `validation_summary.json`  
- `app/src/main/assets/rag/sarathi_rag.sqlite`, `sarathi_rag_manifest.json`
...
- `knowledge/sources/gita/processed/gita_missing_verses_report.json` — gap / remediation notes  
- `docs/RAG_INTEGRATION.md`, `docs/ANDROID_DEV_SETUP.md`  
