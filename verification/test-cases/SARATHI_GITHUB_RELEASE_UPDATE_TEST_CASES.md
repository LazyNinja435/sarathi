...
2. **APP_ONLY:** `.\scripts\package-github-release.ps1 -ReleaseType APP_ONLY` (optional: `-VersionName 0.1.1 -VersionCode 2`).
3. **FULL_MODEL:** `.\scripts\package-github-release.ps1 -ReleaseType FULL_MODEL`.
4. **MODEL_ONLY:** `.\scripts\package-github-release.ps1 -ReleaseType MODEL_ONLY` (output under `dist/github-model-release/`).
...
2. **APP_ONLY:** `.\scripts\publish-github-release.ps1 -ReleaseType APP_ONLY -ClobberUpload` (only if re-uploading assets).
3. **FULL_MODEL:** `.\scripts\publish-github-release.ps1 -ReleaseType FULL_MODEL -ClobberUpload`.
4. **MODEL_ONLY:** `.\scripts\publish-github-release.ps1 -ReleaseType MODEL_ONLY -Tag model-gemma-4-e2b -ClobberUpload` (tag must match the packaged model manifest).
...
1. Publish a new **MODEL_ONLY** release (new tag or updated `model-latest.json` with a newer `model.version` / SHA) following `docs/GITHUB_RELEASE_BOOTSTRAP_PLAN.md`.
