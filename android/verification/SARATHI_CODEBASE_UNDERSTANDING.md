...
| `android/app/src/main/java/com/sarathi/app/` | Main app source packages. |
| `android/app/src/main/assets/rag/` | Bundled RAG SQLite DB and manifest shipped in APK assets. |
| `android/app/src/main/assets/verses.json` | Legacy/fallback verse-of-day asset. |
| `android/docs/` | Maintainer and architecture docs: model setup, RAG, updates, release signing, product design. |
| `android/scripts/` | Windows PowerShell build, install, package, publish, and signing helpers. |
...
| `shared/knowledge/` | RAG source, processed corpus, indexes, validation reports. Stable data; do not casually edit. |
| `android/verification/` | QA reports, smoke scripts, screenshots, test cases, log captures. |
...
Ignored/generated artifacts include `.gradle/`, `.idea/`, `local.properties`, virtualenvs, `*.task`, `*.litertlm`, `models/`, `android/app/src/main/assets/models/`, `local-models/`, `dist/`, `verification/*.log`, APK/AAB/build outputs, keystores, `signing.properties`, and `release-secrets/`.
...
| Path | Purpose |
| --- | --- |
| `android/app/src/main/assets/rag/sarathi_rag.sqlite` | SQLite FTS5 RAG database in APK assets. |
| `android/app/src/main/assets/rag/sarathi_rag_manifest.json` | Small manifest with counts and generation date. |

Source/build corpus:

| Path | Purpose |
| --- | --- |
| `shared/knowledge/sources/` | Raw and processed source texts. |
| `shared/knowledge/indexes/` | Built SQLite, manifest, build and validation reports. |
...
| Path | Role |
| --- | --- |
| `android/scripts/package-github-release.ps1` | Builds APP_ONLY/FULL_MODEL/MODEL_ONLY assets; requires local model even for APP_ONLY metadata. |
| `android/scripts/publish-github-release.ps1` | Publishes assets with `gh`; app/full releases use `--latest`, model-only uses `--latest=false`. |
...
.\android\scripts\package-github-release.ps1 -ReleaseType APP_ONLY
.\android\scripts\package-github-release.ps1 -ReleaseType FULL_MODEL
.\android\scripts\package-github-release.ps1 -ReleaseType MODEL_ONLY
...
```powershell
python android\verification\sqlite_rag_check.py
...
- `android/app/src/main/java/com/sarathi/app/viewmodel/ChatViewModel.kt`
- `android/app/src/main/java/com/sarathi/app/llm/PromptBuilder.kt`
- `android/app/src/main/java/com/sarathi/app/llm/ModelManager.kt`
- `android/app/src/main/java/com/sarathi/app/llm/LiteRtLmGemmaChatEngine.kt`
- `android/app/src/main/java/com/sarathi/app/update/ReleaseManifest.kt`
- `android/app/src/main/java/com/sarathi/app/modeldownload/ModelDownloadManager.kt`
- `android/app/src/main/java/com/sarathi/app/model/InstalledModelInfo.kt`
- `android/docs/GITHUB_RELEASE_DISTRIBUTION.md`
- `android/docs/IN_APP_UPDATES.md`
- `android/docs/PRODUCT_DESIGN.md`
- `android/docs/RAG_INTEGRATION.md`
