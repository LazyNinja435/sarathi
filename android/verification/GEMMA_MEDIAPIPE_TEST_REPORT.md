...
| 2 | Gemma Model Acquisition | No `.task` binary downloaded: Hugging Face resolve URL returns **401** without auth/terms. Manual steps in `android/docs/GEMMA_MODEL_DOWNLOAD.md`. Local holding dir: `local-models/` (ignored). |
...
| 5 | Emulator Install & QA | `:app:installDebug` **SUCCESS**; Settings shows **Missing** model; chip-driven chat exercise; screenshots under `verification/screenshots/gemma/`; logcat capture at `android/verification/gemma_mediapipe_logcat.txt` (git-ignored). |
| 6 | Documentation | `android/docs/MODEL_SETUP.md` updated; added `android/docs/GEMMA_MODEL_DOWNLOAD.md`, `android/docs/GEMMA_MEDIAPIPE_TESTING.md`, `verification/smoke_gemma_mediapipe_adb.ps1`. |

---

## Baseline Safety Results

- **Git:** Repository was **not** initialized; `git init` performed. `.gitignore` updated per milestone (including `*.task`, `models/`, `local-models/`, `.venv/`, `android/verification/gemma_mediapipe_logcat.txt`, `__pycache__/`).
...
- **Instructions:** See **`android/docs/GEMMA_MODEL_DOWNLOAD.md`**.
...
| Logcat (local, ignored by git) | `android/verification/gemma_mediapipe_logcat.txt` |
...
- `.gitignore` — expanded ignores + `android/verification/gemma_mediapipe_logcat.txt`
- `android/docs/MODEL_SETUP.md` — recommended small model, Option C path, HF pointer
- `android/docs/GEMMA_MODEL_DOWNLOAD.md` — **new**
- `android/docs/GEMMA_MEDIAPIPE_TESTING.md` — **new**
