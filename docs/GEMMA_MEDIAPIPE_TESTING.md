...
- **Model:** MediaPipe-compatible `.task` (see `docs/GEMMA_MODEL_DOWNLOAD.md`). **Do not** commit the file to git.
...
adb logcat -c
# exercise app
adb logcat -d > verification\gemma_mediapipe_logcat.txt
```
...
- **HF gated models** return 401 without auth; follow `docs/GEMMA_MODEL_DOWNLOAD.md`.
