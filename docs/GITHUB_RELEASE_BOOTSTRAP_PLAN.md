...
2. Package: `.\scripts\package-github-release.ps1 -ReleaseType MODEL_ONLY`
3. Publish: `.\scripts\publish-github-release.ps1 -ReleaseType MODEL_ONLY -Tag model-gemma-4-e2b` (add `-ClobberUpload` only when replacing assets on an existing tag). New creates pass **`--latest=false`** so this release does not become the repository default “Latest” over the app line.
...
1. Package: `.\scripts\package-github-release.ps1 -VersionName 0.1.0 -VersionCode 1 -ReleaseType APP_ONLY`
...
3. Publish the app tag (for example `v0.1.0`) with `.\scripts\publish-github-release.ps1 -VersionName 0.1.0 -VersionCode 1 -ReleaseType APP_ONLY -Tag v0.1.0` (after configuring signing for the packaging step). New creates pass **`--latest=true`** so `releases/latest/download/sarathi-latest.json` tracks this APK release.
